import { loadInputCommaSeperated } from "../helpers";

export function part1(): string {
  const instructions = loadInputCommaSeperated(2);
  ...
  instructions.forEach((i) => {
    ...
    for (let pId = start; pId <= end; pId++) {
      ...
      // Skip if its not a even number of digits
      if (!(numberofDigits % 2 == 0)) continue;

      // split the number in half
      const firstHalf = pId.toString().slice(0, numberofDigits / 2)
      const secondHalf = pId.toString().slice(numberofDigits / 2)
      // console.log(`${pId} - ${firstHalf} : ${secondHalf}`)

      if (firstHalf === secondHalf) invalidIdsSum += pId
    }


  })
  return `invalidIdsSum : ${invalidIdsSum}`;
}
